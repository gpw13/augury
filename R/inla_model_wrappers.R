...
grouped_predict_inla_ts <- function(df,
                                    group_col = "iso3",
                                    model = "rw2",
                                    formula = as.formula(sprintf("value ~ f(year_n, model = '%s')", model)),
                                    family = "gaussian",
...
predict_inla_me <- function(df,
                            model = "rw2",
                            formula = as.formula(sprintf("value ~ sdi_scaled + e0_scaled + f(region, model = 'iid') + f(year_n, model = '%s')", model)),
                            family = "gaussian",
                            control.predictor = list(compute = TRUE),
                            control.inla = list(strategy = "laplace"),
                            ...,
